...
@private consume_vlq :: proc "fastcall" (this: ^MIDI) -> (out: int, err: MIDI_Error_Code) {
    assert_contextless(this != nil)
    out = 0
    for {
...
@private peek_vlq :: proc "fastcall" (this: ^MIDI) -> (out: int, err: MIDI_Error_Code) {
    old := this.src_ptr
...
@private time_to_seconds :: proc(this: ^MIDI, midi_time: int) -> (Float) {
    assert(this != nil)
...
@private consume_event :: proc(this: ^MIDI) -> (ev: MIDI_Event, err: MIDI_Error_Code) {
    assert(this != nil)
    t_before, t_err := consume_vlq(this)
    if t_err != .NONE {
        return {variant = nil}, t_err
    }
    ev.time = Float(time_to_seconds(this, t_before))
...
init_from_memory :: proc(this: ^MIDI, src: []byte, allocator := context.allocator) -> (MIDI_Error_Code) {
    assert(this != nil)
    this^ = {}
...
init :: proc(this: ^MIDI, fullpath: string, allocator := context.allocator) -> (MIDI_Error_Code) {
    assert(this != nil)
    f_data, ok := os.read_entire_file_from_path(fullpath, allocator)
    if ok != os.ERROR_NONE {
...
parse_realtime :: proc(this: ^MIDI, seconds: Float, allocator: runtime.Allocator, no_alloc := false) -> ([][dynamic]MIDI_Event, MIDI_Error_Code) {
    assert(this != nil)
    cur_track := 0
    ...
    for cur_track < this.num_tracks {
        ...
        for {
            ...
            time_in_seconds := time_to_seconds(this, time_before_next_event)
...
parse_entire_file :: proc(this: ^MIDI, allocator := context.allocator) -> ([][dynamic]MIDI_Event, MIDI_Error_Code) {
...
dump_json :: proc(data: [][dynamic]MIDI_Event, path: string, allocator: runtime.Allocator) {
    ...
    if data != nil {
        ...
        write_err := os.write_entire_file_from_bytes(path, data)
        if write_err != os.ERROR_NONE {
            panic("Could not dump_json")
        }
    }
}